...
test_that("get_argentine_senators() returns a valid tibble with correct structure", {
  result <- get_argentine_senators()
  ...
  expected_columns <- c("id", "nombre", "provincia", "partido", "inicio", "fin")
  ...
  expect_type(result$partido, "character")
  expect_s3_class(result$inicio, "Date")
  expect_s3_class(result$fin, "Date")

  # Check that 'inicio' dates are sorted in descending order
