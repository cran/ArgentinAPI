...
#' Get Country Information for Argentina
#'
#' @description
#' Retrieves comprehensive country information for Argentina from the REST Countries API.
...
#'
#' @return A tibble with one row containing Argentina's country information:
#' \describe{
...
#' for Argentina using full text search. It handles API errors gracefully and
...
#'
#' @export
get_country_info_ar <- function() {
  url <- "https://restcountries.com/v3.1/name/argentina?fullText=true"
  ...
  if (length(data_list) == 0) {
    message("No data found for Argentina.")
    return(NULL)
  }
